...
def score_candidates(candidates: list[dict[str, Any]], taste_profile: dict[str, Any], config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    favorite_authors = {primary_author_key(author) for author, _count in taste_profile.get("favorite_authors", [])}
    ...
    for candidate in candidates:
        ...
        available = any("대출가능" in h.get("availability", "") for h in holdings)
        ...
        author_match = author_key in favorite_authors
        ...
        taste_match = min(40, 8 + (18 if author_match else 0) + (10 if title_related else 0) + (8 if wishlist_related else 0) + len(keyword_hits) * 3)
        thirty_page_pull = 8 + (5 if author_match else 0) + (4 if title_related or wishlist_related else 0) + min(3, len(keyword_hits))
        freshness = 12 if not author_match else 8
        ...
        scored.append(
            {
                **candidate,
                ...
                "score_reason": build_reason(author_match, title_related, wishlist_related, keyword_hits, available, preferred),
                "reasoning_sources": ["portfolio_stats", "candidate_metadata", "library_holdings"],
                "confidence": "medium" if author_match or title_related or wishlist_related or keyword_hits else "low",
                "inference_notes": "도서관 메타데이터와 독서 포트폴리오 통계 기반 자동 점수입니다. 작품 분위기 단정은 Codex 검토가 필요합니다.",
                "taste_matches": (["선호 작가"] if author_match else []) + (["고평점 제목 유사"] if title_related else []) + (["최근 관심 도서 유사"] if wishlist_related else []) + keyword_hits,
                "thirty_page_test": "첫 장에서 문체, 서사 진입 속도, 설명 밀도를 확인해 30쪽 안에 계속 읽고 싶은지 판단하세요.",
...
def build_reason(author_match: bool, title_related: bool, wishlist_related: bool, keyword_hits: list[str], available: bool, preferred: bool) -> str:
    parts = []
    if author_match:
        parts.append("고평점 작가 신호와 직접 연결됩니다")
    if title_related:
